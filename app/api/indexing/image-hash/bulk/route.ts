...
export async function POST(req: NextRequest) {
  ...
  const { batch = 50 } = (await req.json().catch(() => ({}))) as { batch?: number };
  ...
  for (const l of listings || []) {
    const imgs: string[] = Array.isArray(l.images) ? l.images.filter((u: string) => typeof u === 'string' && !!u) : [];
    ...
    for (const url of t.images) {
      ...
      try {
        ...
        const { error } = await supabase
          ...
          .upsert({ listing_id: t.id, image_url: url, ahash_64: hash }, { onConflict: 'listing_id,image_url' });
        if (!error) upserts++;
        if ((process.env.IMAGE_SEARCH_EMBEDDINGS_ENABLED || '0') === '1') {
          try {
            ...
            await supabase
              ...
              .upsert({ listing_id: t.id, image_url: url, embedding: emb }, { onConflict: 'listing_id,image_url' });
          } catch {
            // ignore
          }
        }
