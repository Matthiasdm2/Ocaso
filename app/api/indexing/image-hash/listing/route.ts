...
async function fetchImageBuffer(url: string, timeoutMs = 4000, maxBytes = 4_000_000): Promise<Buffer | null> {
  try {
...
export async function POST(req: NextRequest) {
  ...
  const images: string[] = Array.isArray(row.images) ? row.images.filter((u: string) => typeof u === 'string' && !!u) : [];
  if (images.length === 0) return NextResponse.json({ success: true, updated: 0 });
  ...
  for (const url of images) {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .upsert({ listing_id, image_url: url, ahash_64: hash }, { onConflict: 'listing_id,image_url' });
      if (!error) updated++;
      ...
      if ((process.env.IMAGE_SEARCH_EMBEDDINGS_ENABLED || '0') === '1') {
        try {
          ...
          await supabase
            ...
            .upsert({ listing_id, image_url: url, embedding: emb }, { onConflict: 'listing_id,image_url' });
        } catch {
