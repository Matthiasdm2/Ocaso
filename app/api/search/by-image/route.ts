...
export async function POST(req: Request) {
  ...
  const json: { ok?: boolean; results?: Array<{ listing_id?: string; score?: number }> } = await r.json();
  ...
  if (r.ok && json && Array.isArray(json.results)) {
    ...
    const rawIds: string[] = (json.results as Array<{ listing_id?: string }>)
      .map((h) => h?.listing_id)
      .filter((v): v is string => typeof v === 'string');
    console.log('[by-image] ANN results:', json.results.length, 'raw ids:', rawIds.slice(0, 5));
    ...
    if (validIds.length > 0) {
      const { data } = await supabase
        ...
        .in('id', validIds);
      const existing = new Set<string>((data || []).map((r: { id: string }) => r.id));
      const ordered = validIds.filter((id) => existing.has(id));
      console.log('[by-image] existing ids:', existing.size, 'filtered ordered:', ordered.length);
