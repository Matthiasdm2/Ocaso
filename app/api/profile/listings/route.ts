...
export async function GET(req: Request) {
  ...
  const { data, count, error } = await supabase
    .from("listings")
    .select(
      "id,title,description,price,images,main_photo,created_at,views,category_id,subcategory_id,categories,state,location,allowoffers,status,stock",
      { count: "exact" },
    )
  ...
  for (const l of (data as ListingRow[] | null) ?? []) {
    ...
    if (
      (!l.subcategory_id || typeof l.subcategory_id !== "number") &&
      Array.isArray(l.categories) && l.categories.length > 1
    ) {
  ...
  for (const l of (data as ListingRow[] | null) ?? []) {
    ...
    if (
      !subcategory_id && Array.isArray(l.categories) && l.categories.length > 1
    ) {
    ...
    items.push({
      ...
      imageUrl: l.main_photo ??
        (Array.isArray(l.images) && l.images[0] ? l.images[0] : null),
      images: l.images ?? [],
      ...
      category: category_id
        ...
        : (Array.isArray(l.categories) && l.categories.length > 0
          ? nameMap[l.categories[0]] ?? null
          : null),
      subcategory: subcategory_id
        ? nameMap[subcategory_id] ?? null
        : (Array.isArray(l.categories) && l.categories.length > 1
          ? nameMap[l.categories[1]] ?? null
          : null),
