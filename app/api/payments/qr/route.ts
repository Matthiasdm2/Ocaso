...
export async function POST(request: Request) {
  ...
  if (sellerId === user.id) {
    return NextResponse.json({ error: 'self_request_forbidden', detail: 'Je kan jezelf geen betaalverzoek sturen.' }, { status: 400 });
  }
  ...
  const bank = (seller as { bank?: { iban?: string; bic?: string } | null }).bank || null;
  const rawIban = bank && typeof bank.iban === 'string' ? bank.iban : '';
  const rawBic = bank && typeof bank.bic === 'string' ? bank.bic : '';
  const rawName = ((seller as { display_name?: string | null; full_name?: string | null }).display_name
  ...
  if (!iban) return NextResponse.json({ error: 'seller_missing_iban' }, { status: 400 });
  ...
  const png: Buffer = await QRCode.toBuffer(epc, { type: 'png', errorCorrectionLevel: 'L', scale: 8, margin: 4, color: { dark: '#000000', light: '#FFFFFF' } });
  ...
  if (!recipientId) {
    return NextResponse.json({ error: 'recipient_missing', detail: 'Geen geldige ontvanger voor bericht.' }, { status: 400 });
  }
  ...
  const messageText = "Scan deze code via de banking app en betaal 'het overeengekomen' bedrag.";
  // Insert message using user-scoped client (RLS requires sender to be the authed user)
  const baseMessage = { conversation_id: conversationId, sender_id: user.id, body: messageText } as Record<string, unknown>;
  // Try with listing_id (some DBs have NOT NULL on messages.listing_id)
  let userAttempt = await supabase
    .from('messages')
    .insert({ ...baseMessage, listing_id: listingId, recipient_id: recipientId })
    .select('id, created_at')
    .single();
  let { data: msg, error: msgErr } = userAttempt;
  if (msgErr || !msg) {
    const errText = msgErr?.message || '';
  const missingListing = errText.includes('column') && errText.includes('listing_id');
    const missingRecipient = errText.includes('column') && errText.includes('recipient_id');
    const nullRecipient = errText.includes('null value') && errText.includes('recipient_id');
  if (missingListing) {
      // Retry without listing_id for schemas that don't have this column
      userAttempt = await supabase
        .from('messages')
        .insert(missingRecipient || nullRecipient ? baseMessage : { ...baseMessage, recipient_id: recipientId })
        .select('id, created_at')
        .single();
  ...
  if (msgErr || !msg) {
    console.warn('payments/qr message insert failed via user client; retrying with admin:', msgErr?.message);
    ...
    if (msgErr || !msg) {
      ...
      if (msgErr || !msg) {
        console.error('payments/qr message insert failed (admin fallback):', msgErr?.message);
        return NextResponse.json({ error: 'message_failed', detail: msgErr?.message || 'insert_failed' }, { status: 500 });
      }
    }
  }
  ...
  const { error: upErr } = await admin.storage.from(CHAT_BUCKET).upload(path, png, { contentType: 'image/png', upsert: false });
  ...
  try {
    const { error: attErr } = await admin
      ...
      .insert({ message_id: (msg as { id: string }).id, storage_path: path, mime_type: 'image/png', size_bytes: png.length });
    if (attErr) {
      // Try legacy columns
      await admin.from('message_attachments').insert({ message_id: (msg as { id: string }).id, url: publicUrl || path, content_type: 'image/png' });
    }
