...
export async function GET() {
  ...
  const [profiles, listings, categories, subcategories, bids, favorites, conversations, messages, orders, reviews, user_subscriptions] = await Promise.all([
    count("profiles"),
    count("listings"),
    count("categories"),
    count("subcategories"),
  ...
  let listing_views: { count: number; error: string | null } = { count: 0, error: null };
  try {
