...
export async function POST(req: Request) {
  ...
  const ext = await (accounts as any).createExternalAccount(accountId, { external_account: bankToken });

    return NextResponse.json({ external_account: ext });
  } catch (e) {
    console.error('external-account error', e);
    const message = e instanceof Error ? e.message : 'onbekende fout';
    return NextResponse.json({ error: 'external_failed', details: message }, { status: 500 });
  }
}
