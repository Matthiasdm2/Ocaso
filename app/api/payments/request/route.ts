...
export async function POST(request: Request) {
  ...
  if (sellerId === user.id) {
    return NextResponse.json({
      error: "self",
      detail: "Je kan jezelf geen verzoek sturen.",
    }, { status: 400 });
  }
  ...
  const bodyText = shippingMode === "ship"
    ? "De koper vraagt een betaalverzoek"
    : "De koper vraagt een betaalverzoek (afhalen)";
  ...
  if (msgErr) {
    ...
    if (
      /column .*recipient_id/i.test(errText) ||
      /null value in column "recipient_id"/i.test(errText)
    ) {
