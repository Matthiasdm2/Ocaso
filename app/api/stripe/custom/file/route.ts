...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      const fileField = form.get('file') || form.get('upload') || form.get('image');
    ...
    if (!fileBuffer || fileBuffer.length === 0) return NextResponse.json({ error: 'empty_file' }, { status: 400 });
    if (fileBuffer.length > maxBytes) return NextResponse.json({ error: 'file_too_large' }, { status: 413 });
    const allowed = ['image/jpeg', 'image/png', 'application/pdf'];
    if (contentType && !allowed.includes(contentType.split(';')[0])) return NextResponse.json({ error: 'invalid_mime' }, { status: 400 });

    // Stream to Stripe Files API when possible
    try {
      const stream = await bufferToStream(fileBuffer);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const stripeFile = await (stripe.files as any).create({ file: { data: stream, name: filename, type: contentType }, purpose: 'identity_document' });
      return NextResponse.json({ file: stripeFile });
    } catch (err) {
      // Last resort: upload as buffer
      try {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const stripeFile = await (stripe.files as any).create({ file: { data: fileBuffer, name: filename, type: contentType }, purpose: 'identity_document' });
        return NextResponse.json({ file: stripeFile });
      } catch (e) {
