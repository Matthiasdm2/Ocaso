...
export async function GET() {
  ...
  let html = await readFile(file, "utf8");
    ...
    html = html.replaceAll("{{ .ConfirmationURL }}", url).replaceAll("{{ .SiteURL }}", site).replaceAll("{{year}}", String(new Date().getFullYear()));
    return new NextResponse(html, {
      status: 200,
      headers: { "Content-Type": "text/html; charset=utf-8" },
    });
  } catch (e) {
    return NextResponse.json({ error: "Failed to render preview", details: String(e) }, { status: 500 });
  }
}