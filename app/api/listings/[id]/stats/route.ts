  ...
  try {
    ...
    try {
      ...
      if (!error && data) {
        views = typeof data.views === "number"
          ? data.views
          : Number(data.views ?? 0);
        hasFavoritesCountCol = Object.prototype.hasOwnProperty.call(
          data,
          "favorites_count",
        );
        favoritesFromCol =
          hasFavoritesCountCol && typeof data.favorites_count === "number"
    ...
    if (views === null || !Number.isFinite(views)) {
      try {
        ...
        const { data }: { data: ListingRow } = await sSrv
          .from("listings")
          .select("views, favorites_count")
          .eq("id", listingId)
          .single();
        views = typeof data?.views === "number"
          ? data.views
          : Number(data?.views ?? 0);
        hasFavoritesCountCol = !!data &&
          Object.prototype.hasOwnProperty.call(data, "favorites_count");
        favoritesFromCol =
          hasFavoritesCountCol && typeof data?.favorites_count === "number"
            ? Number(data?.favorites_count)
            : hasFavoritesCountCol
            ? Number(
              (data as { favorites_count?: unknown } | null)?.favorites_count ??
                0,
            )
            : null;
      } catch {
