...
const STOPWORDS = new Set([
  'de','het','een','en','of','voor','met','op','in','te','van','bij','naar','aan','als','je','jij','u','ik','hij','zij','wij','jullie','ze','dit','dat','die','dan','dus','om','maar','ook','nog','al','er','geen','wel','uit','door','tot','over','onder','boven','achter','vooran','voorin','voorop',
  'the','a','an','and','or','for','with','on','in','to','of','by','at','from','is','are','was','were','this','that','these','those','it','as','be'
]);
...
async function findSimilarListingIdsByImage(uploadBuffer: Buffer, limit = 24) {
  ...
  if ((process.env.IMAGE_SEARCH_EMBEDDINGS_ENABLED || '0') === '1') {
    try {
      ...
      const { data: matches } = await supabase.rpc('match_listing_embeddings', { query_embedding: emb, match_count: limit });
      const MAX_DIST = Number(process.env.IMAGE_SEARCH_EMBEDDING_MAX_DIST || 0.35);
      const filtered = (matches || []).filter((m: { distance: number }) => typeof m.distance === 'number' && m.distance <= MAX_DIST);
...
export async function POST(request: NextRequest) {
  ...
  if ((!ids || ids.length === 0) && OCR_ENABLED) {
    ...
    const OCR_TIMEOUT_MS = Number(process.env.IMAGE_SEARCH_OCR_TIMEOUT_MS || 5000);
        const ocrPromise = Tesseract.recognize(uploadBuffer, 'eng+nld', { logger: () => {} }) as unknown as Promise<{ data: { text: string } }>;
    ...
    return NextResponse.json({ success: true, ids, compared, recognized_text, search_terms, query });
  } catch (error) {
