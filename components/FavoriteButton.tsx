...
export default function FavoriteButton({ id, onFavorite }: Props) {
  ...
  const toggle = async () => {
    ...
    try {
      ...
      const headers = await authHeaders();
      // If no auth headers are present, show login prompt instead of attempting API call
  ...
  return (
    <>
      ...
      >
        <Heart
          className={`h-4 w-4 ${isFav ? "text-red-500" : "text-white"}`}
      ...
      {showLoginPrompt && <LoginPrompt onClose={() => setShowLoginPrompt(false)} message="Je moet ingelogd zijn om favorieten te beheren." />}
    </>
  );
}
