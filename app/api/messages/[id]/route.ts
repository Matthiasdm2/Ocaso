...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  const { data: conv } = await supabase
    ...
    .maybeSingle();
  if (!conv) {
    if (process.env.NODE_ENV !== 'production') console.debug('[messages/:id GET] conversation not found or no access', params.id);
  ...
  if (error && /column .* does not exist/i.test(error.message)) {
    if (process.env.NODE_ENV !== 'production') console.warn('[messages/:id GET] fallback to legacy columns');
    let legacy = supabase
      .from("messages")
  ...
  try {
    ...
    if (other) {
      const { data: readRow } = await supabase
        ...
        .eq('user_id', other)
        .maybeSingle();
      if (readRow?.last_read_at) peer_last_read_at = readRow.last_read_at as string;
    }
  } catch { /* ignore */ }

  // Optional listing details (title + main photo) for chat header display
  let listingSummary: { id: string; title: string; image: string | null } | null = null;
  ...
  if (listingId) {
    try {
      const { data: listing, error: lerr } = await supabase
        ...
        .maybeSingle();
      if (!lerr && listing) {
        const images = Array.isArray((listing as { images?: unknown }).images) ? (listing as { images?: string[] }).images : [];
        const image = (listing as { main_photo?: string | null }).main_photo || (images && images.length ? images[0] : null);
  ...
  return NextResponse.json({ messages: messagesWithMeta, hasMore, listing_id: listingId, listing: listingSummary, peer_last_read_at, debug_reads });
}
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  const { data: conv } = await supabase
    ...
    .maybeSingle();
  if (!conv) {
    if (process.env.NODE_ENV !== 'production') console.debug('[messages/:id POST] conversation not found or no access', params.id);
    return NextResponse.json({ error: "not_found" }, { status: 404 });
  }
  if (!Array.isArray(conv.participants) || !conv.participants.includes(user.id)) {
    if (process.env.NODE_ENV !== 'production') console.debug('[messages/:id POST] user not participant', { conv, user: user.id });
  ...
  if (listingId) basePayload.listing_id = listingId;
  {
    const { data, error } = await supabase
      ...
      .select('id, sender_id, body, created_at, edited_at, deleted_at')
      .single();
    insertError = error as unknown; inserted = (data as typeof inserted) || null;
  }
  // Retry if recipient_id constraint appears
  const needsRecipient = (err: unknown) => typeof err === 'object' && err !== null && 'message' in err && /null value in column "recipient_id"/i.test(String((err as { message?: string }).message));
  if (!inserted && needsRecipient(insertError)) {
  const parts = (conv as { participants?: string[] }).participants;
  const other = Array.isArray(parts) ? (parts.find(p => p !== user.id) || null) : null;
    if (other) {
  ...
  .select('id, sender_id, body, created_at, edited_at, deleted_at')
  .single();
      insertError = error2 as unknown; inserted = (data2 as typeof inserted) || null;
    }
  }
  ...
  const needsListing = (err: unknown) => typeof err === 'object' && err !== null && 'message' in err && /null value in column "listing_id"/i.test(String((err as { message?: string }).message));
  if (!inserted && needsListing(insertError) && listingId && !('listing_id' in basePayload)) {
    const retry2 = await supabase
      .from('messages')
      .insert({ ...basePayload, listing_id: listingId })
      .select('id, sender_id, body, created_at, edited_at, deleted_at')
      .single();
    insertError = retry2.error as unknown; inserted = (retry2.data as typeof inserted) || null;
  }
  function isInsertedMessage(v: unknown): v is InsertedMessage {
    return typeof v === 'object' && v !== null &&
      'id' in v && 'sender_id' in v && 'body' in v && 'created_at' in v;
  }
  const msg: InsertedMessage | null = isInsertedMessage(inserted) ? inserted : null;
  const error = (insertError && typeof insertError === 'object' && 'message' in insertError) ? (insertError as { message: string }) : null;
  if (error) {
  if (process.env.NODE_ENV !== 'production') console.error('[messages/:id POST] insert error', error.message);
  // Add hint for common RLS failure
  const hint = /row-level security/.test(error.message) ? 'RLS: zorg dat user in participants staat en policies zijn toegepast' : undefined;
  ...
  return NextResponse.json({ ok: true, message: msg ? { id: msg.id, sender_id: msg.sender_id, body: msg.body, created_at: msg.created_at, edited_at: msg.edited_at, deleted_at: msg.deleted_at, attachments: insertedAttachments } : null, attachments_error: attachmentsError });
}
...
export async function PATCH(request: Request) {
  ...
  if (!messageId || !newText) return NextResponse.json({ error: 'invalid' }, { status: 400 });
  const { data: msg } = await supabase.from('messages').select('id, sender_id, created_at, deleted_at').eq('id', messageId).maybeSingle();
  if (!msg || msg.sender_id !== user.id) return NextResponse.json({ error: 'forbidden' }, { status: 403 });
  if (msg.deleted_at) return NextResponse.json({ error: 'deleted' }, { status: 400 });
  if (Date.now() - new Date(msg.created_at).getTime() > 5 * 60 * 1000) return NextResponse.json({ error: 'window_closed' }, { status: 400 });
  const { error } = await supabase.from('messages').update({ body: newText, edited_at: new Date().toISOString() }).eq('id', messageId);
