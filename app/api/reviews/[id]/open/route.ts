...
export async function POST(
  ...
  try {
    ...
    if (!reviewId) {
      return NextResponse.json({ error: "Review id ontbreekt" }, {
        status: 400,
      });
    }

    // Best-effort insert (ignore duplicate)
    const { error } = await supabase.from("review_opens").upsert({
      user_id: user.id,
      review_id: reviewId,
    ...
    if (error) {
      ...
      if (error.message.includes("does not exist") || error.code === "42P01") {
        console.warn(
          "review_opens table does not exist, skipping review open tracking",
        );
      } else {
