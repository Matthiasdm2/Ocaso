...
export default function InfoPage() {
  ...
  async function save() {
    ...
    try {
      ...
      // Bouw een payload met ENKEL bestaande kolommen in 'profiles'
      const dbPayload = {
        id: user.id,
      ...
      // Gebruik server API met service role voor robuuste upsert
      ...
      if (!r.ok) {
        // Diagnostic: capture server response body and show a toast so we know why it failed
        ...
        try {
          // best-effort fallback: try to persist using the client-side supabase (RLS must allow this)
          // eslint-disable-next-line @typescript-eslint/no-var-requires
          const { createClient: _create } = require('@/lib/supabaseClient');
          const _supabase = _create();

          // Use unknown typing and inspect result for error
          // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
          const upsertRes: unknown = await _supabase.from('profiles').upsert(dbPayload as unknown);
          // Detect Supabase error shape if present
  ...
  const resp = await r.json().catch(() => ({} as { profile?: {
      ...
      // Event om UI te laten refreshen (badges/avatar etc.)
      if (typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('ocaso:profile-updated', { detail: { profile: resp?.profile || dbPayload } }));
      }
      // Haal de actuele waarden opnieuw op zodat de UI zeker synchroon is
      try {
  ...
  if (row && typeof row === 'object') {
      ...
      alert('Opslaan mislukt. Controleer je invoer en probeer opnieuw.');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleAvatarFiles(files: File[]) {
    ...
    try {
      ...
      // Bewaar in profiel (avatar_url kolom) en verifieer – los rapporteren
      try {
        // 1) Probeer update + returning
        const { data: updated, error: updErr } = await supabase
          ...
          .single();
        if (updErr) {
          // 2) Fallback: upsert (mocht de rij ontbreken)
        ...
        // Toon zo veel mogelijk detail zonder any
      ...
      // Laat rest van de UI weten dat avatar gewijzigd is
      if (typeof window !== 'undefined') {
  ...
  // Bereken initialen voor nette placeholder (als er geen avatar is)
  const initials = (() => {
      ...
      <header className="relative border-b">
        ...
        <div className="container mx-auto max-w-6xl px-4 py-10 md:py-14">
          <div className="flex flex-col items-start gap-6 md:flex-row md:items-center">
            {/* Avatar met subtiele wijzig-knop */}
...
// Render consent modal at end of component tree
// (Placed near bottom so it can overlay everything)
// NOTE: Because this is appended after export, move into JSX above if lint complains.
