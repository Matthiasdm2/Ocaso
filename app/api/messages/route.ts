...
export async function GET(request: Request) {
  ...
  const listingIds = Array.from(new Set(rows.map(r => r.listing_id).filter((v): v is string => typeof v === 'string')));
  const listingsMap: Record<string, { id: string; title: string; image: string | null }> = {};
  if (listingIds.length) {
    type ListingRow = { id: string; title: string; main_photo: string | null; images: string[] | null };
...
export async function POST(request: Request) {
  ...
  if (!otherUserId) return NextResponse.json({ error: "missing_otherUserId" }, { status: 400 });
  if (otherUserId === user.id) return NextResponse.json({ error: "self" }, { status: 400 });
  ...
  try {
    ...
    if (listingErr) {
      ...
      if (process.env.NODE_ENV !== 'production') console.warn('[messages POST] listing select error', listingErr.message);
    } else if (!listingRow) {
      return NextResponse.json({ error: 'invalid_listing' }, { status: 400 });
    }
  } catch (e) {
    if (process.env.NODE_ENV !== 'production') console.warn('[messages POST] listing validation exception', e);
  }
  ...
  retry = retry.eq('listing_id', listingId);
    ...
    return NextResponse.json({ error: errorMessage || 'insert_failed' }, { status: 500 });
  }
}
