...
function normalizeRow(r: Record<string, unknown>) {
  ...
  const description = (r['description'] || r['Description'] || r['beschrijving'] || '') as string;
  const location = (r['location'] || r['Location'] || r['plaats'] || '') as string;
  const category = (r['category'] || r['Category'] || r['categorie'] || '') as string;
  const images = (r['images'] || r['Images'] || r['images_csv'] || r['imagesCsv'] || '') as string;
  return { title: String(title || '').trim(), price, description: String(description || '').trim(), location: String(location || '').trim(), category: String(category || '').trim(), images: String(images || '').trim() };
}
...
function isProbablyCsv(text: string) {
  ...
  return commaCount >= 1 && /title|price|description|category/i.test(lines.join('\n'));
}
...
export async function POST(req: Request) {
  ...
  const ab = await req.arrayBuffer();
      if (!ab || (ab as ArrayBuffer).byteLength === 0) return NextResponse.json({ error: 'Geen bestand in body' }, { status: 400 });
  rows = await parseWorkbook(Buffer.from(ab));
    }
    ...
    const results: Array<{ row: number; ok: boolean; error?: string; listingId?: string }> = [];
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        if (!norm.price || norm.price <= 0) throw new Error('Prijs ontbreekt of ongeldig');
        ...
        if (norm.images) {
          const imgs = norm.images.split(',').map(s => s.trim()).filter(Boolean);
        ...
        if (norm.category) {
          ...
          if (!Number.isNaN(maybeId) && maybeId > 0) insertPayload['category_id'] = maybeId;
          else {
            // try find category by slug or name
            const { data: cat } = await supabase.from('categories').select('id,slug,name').or(`slug.eq.${norm.category},name.ilike.%${norm.category}%`).limit(1).maybeSingle();
            if (cat && cat.id) insertPayload['category_id'] = cat.id;
          }
        }

  const { data: inserted, error: insErr } = await supabase.from('listings').insert(insertPayload).select('id').maybeSingle();
  if (insErr || !inserted) throw new Error((insErr && (insErr.message || String(insErr))) || 'Insert failed');
