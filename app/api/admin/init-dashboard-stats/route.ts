...
export async function POST() {
  try {
    ...
    if (fetchError) {
      return NextResponse.json({ error: `Failed to fetch businesses: ${fetchError.message}` }, { status: 500 });
    }

    if (!businesses || businesses.length === 0) {
      return NextResponse.json({ message: "No businesses found to initialize" });
    }
    ...
    for (const business of businesses) {
      try {
        ...
        if (recalcError) {
          results.push({ business_id: business.id, error: recalcError.message });
        } else {
    ...
    return NextResponse.json({
      message: `Initialized dashboard stats for ${successCount} businesses, ${errorCount} errors`,
      results
    });
