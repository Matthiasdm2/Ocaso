...
type SharpModule = (input?: unknown) => {
  resize: (w: number, h: number, o: { fit: 'cover' }) => ReturnType<SharpModule>;
...
export async function computeImageEmbedding(buffer: Buffer): Promise<number[]> {
  ...
  const img = await sharpLib(buffer).resize(224, 224, { fit: 'cover' }).removeAlpha().toFormat('png').toBuffer();
  ...
  const arr = Array.isArray(raw) ? raw as number[] : Array.from((raw as { length: number; [k: number]: number }));
