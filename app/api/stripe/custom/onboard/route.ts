...
export async function POST(req: Request) {
  try {
    ...
    if (
      body.business_profile && Object.keys(body.business_profile).length > 0
    ) {
    ...
    if (tos_acceptance && Object.keys(tos_acceptance).length > 0) {
      accountData.tos_acceptance = tos_acceptance;
    }
    ...
    if (external_account && typeof external_account === "object") {
      ...
      if (token) {
        try {
          ...
          const added = await (accounts as any).createExternalAccount(acct.id, {
            external_account: token,
          });
          const addedAny = added as unknown as Record<string, unknown> | null;
          console.log(
            "Attached external account token to connected account:",
            addedAny?.id ?? JSON.stringify(addedAny),
          );
        } catch (attachErr) {
          console.warn(
            "Failed attaching external account token during onboarding:",
            attachErr,
          );
        }
      } else {
        console.log(
          "External account data received, will be added later:",
          JSON.stringify(external_account, null, 2),
        );
      }
    }
    ...
    return NextResponse.json(
      { error: "onboard_failed", details: errorMessage },
      { status: 500 },
    );
  }
}
