...
export async function GET(
    ...
    console.log(
        `[messages/:id] Checking conversation access for ${
            params.slug[0]
        }, user: ${user?.id}`,
    );
    ...
    if (!conv) {
        console.log(
            `[messages/:id] Access denied or conversation not found for ${
                params.slug[0]
            }`,
        );
    ...
    let query = supabase
        ...
        .eq("conversation_id", params.slug[0])
        ...
        if (ids.length) {
            ...
            if (attErrNew && /column .*storage_path/i.test(attErrNew.message)) {
                ...
                const { data: attsOld } = await supabase
                    ...
                    .in("message_id", ids);
                (attsOld as OldAttRow[] | null | undefined || []).forEach(
                    (a) => {
                ...
                (attsNew as NewAttRow[] | null | undefined || []).forEach(
                    (a) => {
                        if (!attachmentsMap[a.message_id]) {
                            attachmentsMap[a.message_id] = [];
                        }
    ...
    if (listingId) {
        try {
            ...
            if (!lerr && listing) {
                const images =
                    Array.isArray((listing as { images?: unknown }).images)
                        ? (listing as { images?: string[] }).images
                        : [];
                const image =
                    (listing as { main_photo?: string | null }).main_photo ||
...
export async function POST(
    ...
    const attachments: IncomingAttachment[] =
        Array.isArray(bodyJson.attachments)
    ...
    const hasEpcQr = attachments.some((a) => {
        ...
        return name.includes("epc") &&
            (a.mime_type || a.content_type || "").toLowerCase().startsWith(
                "image/",
            );
    });
    ...
    if (!inserted && needsRecipient(insertError)) {
        ...
        if (other) {
            ...
            const { data: data2, error: error2 } = await supabase
                ...
                .insert(retryPayload)
                .select(
                    "id, sender_id, body, created_at, edited_at, deleted_at",
                )
                .single();
    ...
    if (error) {
        ...
        const hint = /row-level security/.test(error.message)
            ...
            : undefined;
        return NextResponse.json({ error: error.message, hint }, {
            status: 400,
        });
    }
    ...
    if (msg && attachments.length) {
        ...
        const legacyOnes = attachments.filter((a) =>
        ...
        if (newOnes.length) {
            try {
                ...
                if (errNew && /column .*storage_path/i.test(errNew.message)) {
                    ...
                    const fallbacks = newOnes.filter((a) => !!a.url).map((
                        a,
                    ) => ({
                        message_id: msg.id,
                        url: a.url!,
                    ...
                    if (fallbacks.length) {
                        const {
                            data: insLegacyFromNew,
                            error: errLegacyFromNew,
                        } = await supabase
                            .from("message_attachments")
                            .insert(fallbacks)
                            .select("id,message_id,url,content_type");
                    ...
                    attachmentsError = errNew.message ||
                        "attachment_insert_failed";
                    if (process.env.NODE_ENV !== "production") {
                        console.error(
                ...
                attachmentsError = (e as Error)?.message ||
                    "attachment_insert_failed";
            }
        }
            ...
            try {
                ...
                if (
                    ...
                    (/column .*url/i.test(errOld.message) ||
                        /null value in column "storage_path"/i.test(
                            errOld.message,
                        ))
                ) {
                    // DB uses new schema only; can't save url-only attachments
                    attachmentsError = errOld.message ||
                        "attachment_insert_failed";
                } else if (errOld) {
                    attachmentsError = errOld.message ||
                        "attachment_insert_failed";
...
export async function DELETE(
    ...
    if (isConversationDelete) {
        ...
        if (msgError) {
            return NextResponse.json({ error: msgError.message }, {
                status: 400,
            });
        }
        const { error: convError } = await supabase.from("conversations")
            ...
            .eq("id", params.slug[0]);
        if (convError) {
            return NextResponse.json({ error: convError.message }, {
                status: 400,
            });
        }
        return NextResponse.json({ ok: true });
